	...
	void handle_client(SOCKET client_sock, int client_id)
	{
		...
		while (running.load())
		{
			...
			if (payload_len == 126)
			{
				...
				if (recv(client_sock, (char *)len_bytes, 2, 0) != 2)
					break;
			...
			if (masked)
			{
				if (recv(client_sock, (char *)mask, 4, 0) != 4)
					break;
			}
			...
			if (extended_len > 0)
			{
				...
				if (payload_received != (int)extended_len)
					break;
...
public:
	...
	bool start(int p) override
	{
		...
		SimuCoreLogger::log("Starting webserver on port " + std::to_string(port));
		return true;
	}
	...
	bool send_message_to_client(int clientId, const std::string &message) override
	{
		std::lock_guard<std::mutex> lock(clients_mutex);
